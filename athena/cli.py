...
async def run_eval(dataset: str, model: str, report: str):
    ...
    for r in rows:
        ...
        pred = gen["output"]
        em = exact_match(pred, r.gold)
        f1 = f1_score(pred, r.gold)
        rl = rouge_l(pred, r.gold)
        ems.append(em)
        f1s.append(f1)
        rls.append(rl)
        records.append(
            {"prompt": r.prompt, "pred": pred, "gold": r.gold, "em": em, "f1": f1}
        )
    write_report(
        report, records, stats.mean(ems), stats.mean(f1s), stats.mean(rls), model
    )
...
@app.command()
def eval(
    dataset: str = "datasets/demo.jsonl",
    model: str = "local-echo",
    report: str = "reports/report.html",
):
